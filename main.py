...
from config import CHECK_INTERVAL, DEPARTMENTS
...
def process_email(email_data: dict):
    ...
    # Step 4: Forward to department (skip if general / same inbox)
    ...
    # Step 5: Mark as read so we don't process it again
    mark_as_read(mail_conn, uid)
...
def run():
    ...
    logger.info(f"Checking inbox every {CHECK_INTERVAL} seconds (batch of 5 per cycle)")
    print("\nEmail Auto-Reply Bot is running. Press Ctrl+C to stop.\n")
    ...
    while True:
        try:
            ...
            if not emails:
                ...
                # Close IMAP connection after batch
